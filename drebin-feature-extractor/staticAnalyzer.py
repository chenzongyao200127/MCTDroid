...
def getActivities(sampleFile):
    ...
    manifest = subprocess.Popen([settings.AAPT, 'dump', 'badging', sampleFile],
                                ...
                                stdin=subprocess.PIPE,
                                stderr=subprocess.PIPE)
    manifest = manifest.communicate()[0].decode("utf-8", "replace").split("\n")
    for line in manifest:
        if "activity" in line:
            try:
                ...
                activity = "." + activity
                # print activity
                activities.append(activity.encode('ascii', 'replace').decode())
            except:
                continue
        else:
            continue
...
def getFilesInsideApk(sampleFile):
    ...
    xml = subprocess.Popen([settings.AAPT, 'list', sampleFile],
                           ...
                           stdin=subprocess.PIPE,
                           stderr=subprocess.PIPE)
    xml = xml.communicate()[0].decode("utf-8", "replace").split("\n")
    for line in xml:
        try:
            files = line.split("\n")[0]
            files = re.compile('[%s]' % re.escape(CC)).sub('', files)
            if files != "":
                appFiles.append(files.encode('ascii', 'replace').decode())
        except:
            continue
    return appFiles
    ...
    for line in xml:
        if "intent" in line:
            try:
                ...
                appIntents.append(intents.encode('ascii', 'replace').decode())
            except:
                continue
        else:
            continue
    ...
    for line in xml:
        if "android.net" in line:
            try:
                ...
                if net != "":
                    appNet.append(net.encode('ascii', 'replace').decode())
            except:
                continue
        else:
            continue
...
def getSampleInfo(logFile, sampleFile):
    ...
    appInfos.append(apkName.encode('ascii', 'replace').decode())
    return appInfos
            ...
            try:
                ...
                if (nextLine not in servicesANDreceiver) and (nextLine != ""):
                    servicesANDreceiver.append(
                        nextLine.encode('ascii', 'replace').decode())
            except:
                continue
        else:
            continue
...
def run(sampleFile, workingDir):
    ...
    # print "get Network data..."   find the package which the family is "android.net"
    appNet = getNet(sampleFile)
    # print "get sample info..."    record the basic information of the apk
