...
def get_fd_vae_feature(apk_path, output_path=None):
    """
    ...
    try:
        ...
        apk_permissions = {permission.split(
            ".")[-1] for permission in a.get_permissions()}
        ...
        logging.error(
            red(f"Error occurred in APK: {os.path.basename(apk_path)}"))
        traceback.print_exc()
    ...
    if output_path is not None:
        np.savez(output_path, fd_vae_feature=total_feature)
        logging.critical(
            green(f'Successfully saved the fd-vae feature in: {output_path}'))
