...
class Detector:
    def __init__(self, name, saving_path, feature, classifier):
        # Initialize the detector with a name, path for saving models, feature type, and classifier type
        self.name = name + ".model"
        self.mlp_epochs = 1
        # Build saving path differently if classifier is a multilayer perceptron
        ...
        # Initialize classifier and vectorizer to None, will be set up in build_classifier
        self.clf = None
        self.vec = None
        # Initialize datasets placeholders
        self.X = None
        self.X_train = None
        self.y_train = None
        self.X_test = None
    ...
    def build_classifier(self, dataset, save=True):
        ...
        # Extract features based on the specified type from the dataset
        ...
        if self.feature == "drebin" or self.feature == "apigraph":
            if os.path.exists(self.saving_path + ".vec"):
                ...
                X = self.vec.transform(X)
        elif self.feature == "malscan" or self.feature == "mamadroid" or self.feature == "vae_fd":
            X = np.asarray(X)
    ...
    def save_to_file(self):
        # Method to save the trained classifier to a file
        with open(self.saving_path + ".clf", "wb") as f:
            pickle.dump(self.clf, f, protocol=4)

        # If the feature requires a vectorizer (for 'drebin' or 'apigraph'), save it as well
        if self.feature == "drebin":
