...
class PerturbationSelectionTree:
    ...
    def print_tree(self):
        wait_queue = []
        wait_queue.append(self.android_root)
        while len(wait_queue):
            current_node = wait_queue.pop(0)
            print("Node {} :".format(current_node.name))
            for node in current_node.children:
                print("Children Node {} - value: {}".format(node.name, node.value))
                if not node.is_leaf:
